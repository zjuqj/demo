...
class FaceAlignmentTools:
    ...
    def detect(self, img, dsize: tuple = None, allow_multiface: bool = True):
        """
        Face detection and alignment of a single image
        ...
        n_src_bboxes = self.detect_face_new(img, allow_multiface=allow_multiface)

        # print(n_src_points)
        if n_src_bboxes is None:
            UserWarning("No face detected! Skipping face alignment!")
            return None

        return n_src_bboxes

    def detect_face_new(self, img: np.ndarray, allow_multiface: bool = True):
        """
        Detects bounding boxes from the specified image.
        ...
        for bbox in bboxes:
            if bbox[-1] < 0.99:
                UserWarning("Face detection score is only {}, consider checking the detected face!".format(bbox[-1]))
        bboxes = np.delete(bboxes, 4, 1)
        # print(bboxes)
        # bboxes = np.reshape(bboxes, [2,2,len(bboxes)])
        # print(bboxes)
